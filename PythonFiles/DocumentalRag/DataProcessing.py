...
class DocumentProcessor:
    ...
    def generate_chunked_dictionaries(self, data_dict, chunk_size=512, overlap_percentage=[0, 0.1, 0.2, 0.3]):
    ...
    def generate_chunked_dictionaries_ii(self, data_dict, chunk_size_large=512, chunk_size_list=[64, 128, 256]):
        dictList = []
...
'''
    def insert_data_ii(self, conn, chunked_dict: dict):
        ...
        try:
            self.create_index_ii(conn)
            self.create_view(conn)
        except Exception as e:
            logging.error(f"Error creating index or view: {e}")
            raise
...
''' 
    def create_schemas(self, conn, schemas):
            """Create schemas in the SQL database using psycopg3"""
            try:
                with conn.cursor() as cur:
    ...
    def wipe_databases(self, dbs_overlap, dbs_hybrid):
        ...
        for db in dbs_hybrid:
            with db.cursor() as cur:
                ...
                cur.execute("DROP VIEW IF EXISTS joined_view")
            db.commit()
    ...
    def init_for_sea(self,overlap_schemas_dict, hybrd_schemas_dict):
        ...
        self.create_schemas(db, schemas_dict.keys())
        logging.info("Created schemas.")
        db.close()
        logging.info("Closed connection to SQL database.")
        
        
        for idx, schema, _ in enumerate(overlap_schemas_dict.items(),0):
            db = psycopg2.connect(
                                    host=os.getenv('HOST'),
                                    dbname=os.getenv('DB_NAME'),
                                    user=os.getenv('USER'),
            ...
            logging.info(f"Connected to SQL database: {schema}")
            self.create_tables(db)
            logging.info(f"Created tables in database: {schema}")
            self.insert_data(db, dict_list[idx])
            logging.info(f"Inserted data into database: {schema}")
            
        logging.info("Overlap Data processing completed.")
        
        
        for idx, schema,_ in enumerate(hybrd_schemas_dict.items(),0):
            db = psycopg2.connect(
                                    host=os.getenv('HOST'),
                                    dbname=os.getenv('DB_NAME'),
                                    user=os.getenv('USER'),
            ...
            logging.info(f"Connected to SQL database: {schema}")
            self.create_tables_ii(db)
            logging.info(f"Created tables in database: {schema}")
            self.insert_data_ii(db, hybrid_list[idx], id)
            logging.info(f"Inserted data into database: {schema}")
            
        logging.info("Hybrid Data processing completed.")
        
        logging.info("Inizialization completed.")
    ...
    def process_data(self):
        ...
        dbs_overlap = [db0, db10, db20, db30]
        ...
        if response == 'yes':
            for db, data_dict in zip(dbs_overlap, dict_list):
                ...
                logging.info(f"Inserted data into database: Overlap {(dbs_overlap.index(db) *10)}% ")
        
        
        dbs_hybrid = [db_hybrid_54, db_hybrid_128,  db_hybrid_256]
        ...
        if response == 'yes':
            for db, data_dict in zip(dbs_hybrid, hybrid_list):
                ...
                logging.info(f"Inserted hybrid data into database: {(2 ** dbs_hybrid.index(db)) * 64}")
